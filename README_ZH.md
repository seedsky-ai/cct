...
> 另有一个隐藏的第四档 **Aligned**(`cct -e aligned`),装的是搜索最新产出的方案。
>
...
> 靠测量模型而不是想象模型选出来的方案。**设计模型真正愿意在里面思考的那个 harness,而不是在它周围搭一个布景。**
>
...
用法就是在你原来的命令前加一个 `cct`：启动时选档，退出时给你一张花费回执。调档位那几档只动一样东西：**模型想得多深**。三个 pro 专属档另外会在系统提示前面拼一段固定方案，
...
小 harness 解得更多。这三个档在 Claude Code 自己的 prompt 之上套一层短方案,让模型**表现得像是在读那套更小的
...
这个 20 由同一条搜索线上的两个不同方案分别达成:`Proven` 在 2026-09 的面板上 20 / 29,更新的搜索方案在
2026-09-07 拿到 20 / 30。
...
`cct` 做的不是这件事。这里装的东西**不是凭直觉手写的,也不是一个人格**,而是**对着模型内部机理搜出来的**。
活下来的那一个,是被测量选中的,不是被品味选中的。
...
- **它是对着一把尺自进化的,不是对着一个故事。** 十几个听起来很合理的想法就是这么死掉的——包括我们自己几个读起来漂亮、测出来更差的。
...
| *(隐藏)* **Aligned** | `cct -e aligned` | 机理搜索最新产出:骨架同 `Proven`,但计划行只陈述对每道题都字面为真的事,并把 `/effort` 压在 `low`,让这段方案成为模型看到的唯一深度指令。不进选择器;2026-09-07 拿到 20 / 30 的就是它 | 1.1 KB |


`Proven` / `Swift` / `Peak` 出自另一条线：它们调的不是思考深度，而是往系统提示前面拼一段
**语域方案**，并且是在**跑到底带验证器的真实仓库任务**（SWE-bench 形态）上量出来的。和上面三档有三点不同：
...
- **它们钉死模型。** 不管 `/model` 显示什么，这三档一律走 `deepseek-v4-pro` —— 因为它们**只**在
  ...
  **两处必须一致**：如果 Claude Code 以别的名字在跑（`settings.json` 的 `env.ANTHROPIC_MODEL`、
  ...
  警告，回执也会点名这些请求。
- **它们不动 `/effort`。** 与调档位的那几档不同，这三档只套用方案、把你的 effort 原样转发 ——
  这正是它们被标定时的构型。
- **它们按 pro 计费。** pro 单价约为 flash 的 3 倍,回执会按 pro 价算。

| 档位 | 方案做什么 | 实测（官方 API，`deepseek-v4-pro`） |
|---|---|---|
| **Proven** | 电报体语域方案 | **29 题解出 20 题** |
| **Swift** | 同语域，外加告诉模型"你只有 shell 和文件编辑器" | 分歧面板上 **8 题解出 7 题** |
| **Peak** | Proven 的同一段文本，每轮再重复一次 | **30 题解出 18–21 题** |

**`Swift` 用文字收窄工具面。** 它的方案写明"你恰好只有两个工具……忽略此提示里描述的其他工具……
...
| `proven` `tm03` `tripara` | **Proven** | 语域方案，仅 pro —— 见[三个 pro 专属档](#三个-pro-专属档) |
| `swift` `qt05` | **Swift** | 同上，外加把工具面收窄到 shell + 编辑器 |
| `peak` `tm03x5` | **Peak** | 同一段方案，每轮重复一次 |
...
模型钉死、思考档位钉死、前导拼接位置这三样都由档位携带，所以用 `DA_PREAMBLE_FILE` 手工挂同一份
方案得到的是**另一个构型** —— 这是有意为之：`DA_PREAMBLE_FILE` 保持它一贯的行为不变。
