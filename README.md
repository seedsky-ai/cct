...
carry a **register profile** measured against agentic SWE tasks (SWE-bench-style repos, run to
...
|---|---|---|
| **Proven** | telegraph-register profile | **20 of 29 tasks.** One run per task, so that number has no ambiguity in it |
...
**How these compare to running with no profile at all — read this before believing the table.**
The honest answer is that the benchmark cannot resolve it. The no-profile arm was run many times,
and 8 of its 15 repeated tasks come out differently on a re-run — a ~50% flip rate. So "Proven
versus no profile" has no single value; it depends on which of the no-profile runs you pick:

| Which no-profile run you compare against | Result | p |
|---|---|---|
...
Same data, same 29 tasks, p from 0.01 to 0.42. **Proven leans better than no profile in every
...
⚠ **`Swift` narrows the tool surface on purpose.** Its profile tells the model "you have exactly two tools…
