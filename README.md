...
>
> **How we took it.** Not by taste, and not by guessing. We read the model from the inside while it
> works — the same instrument our research group built for reading reasoning state — and let that
> reading, not our intuition, tell thinking that is going well from thinking that is merely getting
...
> **Aligned** (`cct -e aligned`), carrying the newest profile the search produced.
>
...
tiers additionally apply a fixed profile, and one of them (`Peak`) adds a fixed reminder each
...
tiers apply a short profile on top of Claude Code's own prompt so that the model **behaves as if it
...
|---|---|---|---|
| system prompt | ~12 KB of agent policy | 46 bytes | nothing is deleted; a profile is applied |
...
The 20 was reached twice, by two different profiles from the same search line: `Proven` in the
2026-09 panel and the newer search-derived profile on 2026-09-07.
...
`cct` is not doing that. What it carries was not written by hand from intuition and it is not a
persona: it was **searched for against the model's own internals**. What shipped is what survived
measurement, not what read well.
...
- **It self-evolves against a ruler, not against a story.** A dozen plausible ideas died that way —
  including several of ours that read well and measured worse.


#### The three tiers themselves

`Proven` / `Swift` / `Peak` all carry a **register profile** — short clipped clauses in the voice the
small harness reasons in — measured against agentic SWE tasks (SWE-bench-style repos, run to
completion with a verifier). They differ only in what the profile says and how often:

| Tier | Pick it with | What it carries | Size |
|---|---|---|---|
...
| *(hidden)* **Aligned** | `cct -e aligned` | the newest profile the mechanism search produced: same skeleton as `Proven`, but the plan line states only things literally true of every task, and `/effort` is held at `low` so the profile is the only depth instruction the model sees. Not in the picker; this is the one that scored 20 / 30 on 2026-09-07 | 1.1 KB |
...
- **They leave `/effort` alone.** Unlike the tuned tiers, these apply their profile and forward
...
| Tier | What the profile does | Measured, official API, `deepseek-v4-pro` |
|---|---|---|
| **Proven** | telegraph-register profile | **20 of 29 tasks** |
...
**`Swift` narrows the tool surface by instruction.** Its profile tells the model "you have exactly two tools…
...
| `proven` `tm03` `tripara` | **Proven** | register profile, pro only — see [the three pro-only tiers](#the-three-pro-only-tiers) |
| `swift` `qt05` | **Swift** | same, with the tool surface narrowed to a shell and an editor |
| `peak` `tm03x5` | **Peak** | same profile, also as a reminder after the first message |
