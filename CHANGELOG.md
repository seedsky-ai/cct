...
- **`aligned` tier (hidden, `cct -e aligned`)**: the profile the mechanism search produced, shipped so
  the 20 / 30 number refers to bytes you can run. It is deliberately **not** in the picker — the
  picker layout is calibrated for six labels and this profile has only a mechanism-level result behind
...
- **Picker goes from three tiers to six.** `Proven` / `Swift` / `Peak` join `Flex` / `Value` / `Deeper`.
  They come from a different line of work than the depth-tuned tiers: each applies a **register
  profile** on top of the system prompt, measured on agentic SWE tasks run to completion with a
...
- The three new tiers carry `keep_effort`, so **`/effort` is forwarded as you set it** — they splice
  their profile and touch nothing else, which is the construction they were calibrated in. The
...
- **Read the tier notes before picking one.** Official-API results, `deepseek-v4-pro`: `Proven`
  ...
  Against running with **no profile at all**, the benchmark cannot resolve the difference: the
  no-profile arm flips outcome on 8 of its 15 repeated tasks, so the comparison ranges from
